<script>
  // Renders one field's identity, runways, and frequency table. Used for the
  // detected airport and for the Departure / Arrival legs of a journey.
  import { activeRunway, com1Mhz, com2Mhz } from './store.js';
  import { sendMessage } from './ws.js';

  export let airport;                 // {icao, name, elevation_ft, runways[], frequencies[]}
  export let showRunways = true;
  ...
  $: sortedFreqs = airport ? [...airport.frequencies].sort((a, b) => {
    const ia = freqTypeOrder.indexOf(a.type_code);
    const ib = freqTypeOrder.indexOf(b.type_code);
    return (ia === -1 ? 99 : ia) - (ib === -1 ? 99 : ib);
  }) : [];

  // Match at the radio's real 10 kHz resolution — an 8.33 kHz frequency like
  // 121.955 stores as 121.95, so a float epsilon on the third decimal misses it.
  const raw = (mhz) => Math.round(mhz * 100);
  function isCom1(freqMhz) { return raw(freqMhz) === raw($com1Mhz); }
  function isCom2(freqMhz) { return raw(freqMhz) === raw($com2Mhz); }
...
{#if airport}
  ...
  <section class="identity">
    ...
    <div class="elev muted">Elev {airport.elevation_ft} ft</div>
  </section>

  {#if showRunways && airport.runways?.length}
    <div class="divider"></div>

    <!-- Runways -->
    <section>
      <div class="section-label">RUNWAYS</div>
      {#each airport.runways as rwy}
