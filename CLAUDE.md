...
- **Test suite is fully mocked.** All tests `@patch` the `atc.engine.respond()` and `atc.engine.boundary_check()` calls — no live Claude or X-Plane connection needed. `test_full_flight_exchange.py` contains `xfail` tests marking en-route/arrival phases as not yet fully implemented. The one exception is `tests/test_live_xplane.py`, an opt-in taxi-routing diagnostic that hits a running X-Plane: skipped unless `VATC_LIVE=1` is set. Run it with `VATC_LIVE=1 .venv/bin/python -m pytest -s tests/test_live_xplane.py` to print the live position, snapped stand/node, and computed route to every runway end.
